...
class IoAudioMixin:
    ...
    def _normalize_audio_to_stereo_48k(self, audio: torch.Tensor, sr: int) -> torch.Tensor:
        ...
        # Peak-normalize to -1 dB to match the energy range the model was
        # trained on.  Raw DAW exports are often significantly louder than
        # model-generated audio, causing out-of-distribution VAE latents
        # that produce garbled diffusion output (especially on xl-base).
        peak = audio.abs().max()
        if peak > 1e-6:
            target_peak = 10 ** (-1.0 / 20.0)  # -1 dB ≈ 0.891
